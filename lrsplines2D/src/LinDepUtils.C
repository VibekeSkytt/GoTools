...
struct MeshRectangle {
  ...
  void construct_first_incidence_matrix( const LRSplineSurface& lrs,
					 ...
					 IncidenceMatrix& m1, 
					 IncidenceMatrix& b1,
					 vector<pair<Element2D*, vector<LRBSpline2D*> > >& elem_bspl,
					 vector<pair<LRBSpline2D*, vector<Element2D*> > >& bspl_elem)
  {
    ...
    elem_bspl.resize(lrs.numElements());
    bspl_elem.resize(lrs.numBasisFunctions());
    int ix_elem = 0;
    for (auto it_el=lrs.elementsBegin();
        it_el!=lrs.elementsEnd(); ++it_el) { // Loop 0: over elements.
      Index in_el = MImap.at(it_el->second.get());
      vector<LRBSpline2D*> tmp(it_el->second->supportBegin(),
			      it_el->second->supportEnd());
      elem_bspl[ix_elem] = make_pair(it_el->second.get(), tmp);
      ++ix_elem;
    ...
    int ix_bspl = 0;
    for (auto it_bspl=lrs.basisFunctionsBegin(); it_bspl!=lrs.basisFunctionsEnd(); ++it_bspl)
      {
	vector<Element2D*> tmp(it_bspl->second->supportedElementBegin(),
			       it_bspl->second->supportedElementEnd());
	bspl_elem[ix_bspl] = make_pair(it_bspl->second.get(), tmp);
	++ix_bspl;
      }
    return;
  }
    ...
    const LRSplineSurface& lrs )
  {
    ...
    vector<pair<Element2D*, vector<LRBSpline2D*> > > elem_bspl;
    vector<pair<LRBSpline2D*, vector<Element2D*> > > bspl_elem;
    construct_first_incidence_matrix( lrs, MImap, BSmap, m1, b1, elem_bspl, bspl_elem);
    ...
    int num_entries = numEntriesOn(fun_is_on);
    if ( num_entries ==0 )
      return result;
    
    // std::ofstream of1("overload_cand1_0.g2");
    // for (BsplineIndexMap::const_iterator it_bs=BSmap.begin();
    // 	 it_bs!=BSmap.end(); ++it_bs)   // Loop over: B-splines
    //   {
    // 	Index ix = it_bs->second;
    //   if (fun_is_on[ix]==ON)      // If this function is ON ...
    // 	{
    // 	  std::cout << it_bs->first << " " << std::endl;
    // 	  LRBSpline2D *cand = it_bs->first;
    ...
    // std::ofstream of2("overload_cand1_1.g2");
    // int num = 0;
    // for (BsplineIndexMap::const_iterator it_bs=BSmap.begin();
    // 	 it_bs!=BSmap.end(); ++it_bs)   // Loop over: B-splines
    //   {
    ...
    // std::cout << std::endl;
    // writeg2Mesh(lrs, of2);
    int num = numEntriesOn(fun_is_on);
    std::cout << "Num preliminary overloads: " << num << std::endl;
