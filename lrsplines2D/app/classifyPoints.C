...
/// Classify point cloud according to distance to approximating 1D surface and colour the
/// points according to specified levels. Green points are below the surface and red
/// points above. White points lie in the level closest to the surface. The final number of
/// levels is at most 2*(nmb_levels+1).
/// If projected distance is set (default 0), distances will be computed in 3D.

int main(int argc, char *argv[])
{
  if (argc != 6 && argc != 7) {
    std::cout << "Usage: surface in (.g2), point cloud (.g2), points_out.g2, max level, nmb_levels, (use projected distance (0/1))" << std::endl;
    return -1;
  }
  ...
  for (ki=0; ki<level_points.size(); ++ki)
    {
      if (ki == 0)
	      std::cout << "Level: [-inf," << limits[ki] << "], no. of pts: " << level_points[ki].size()/3 << std::endl;
      else if (ki == level_points.size()-1)
      std::cout << "Level: [" << limits[ki-1] << ",inf], no. of pts: " << level_points[ki].size()/3 << std::endl;
      else
	std::cout << "Level: ["<< limits[ki-1] << ","  << limits[ki] << "], no. of pts: " << level_points[ki].size()/3 << std::endl;
  ...
  std::cout << "Max dist above: " << maxdist << ", max dist below: " << mindist;
