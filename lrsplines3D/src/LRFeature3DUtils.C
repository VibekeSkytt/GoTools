...
//===========================================================================
{
  ...
  vector<double> elem_avderz(num_elem*nc2, 0.0);
  vector<double> elem_maxderz(num_elem*nc2, std::numeric_limits<double>::lowest());
	  ...
	  for (kb=0; kb<parv.size(); ++kb)
	      ...
	      for (ka=0; ka<paru.size(); ++ka, kd+=10)
		{
		  ...
		  slope2 = sqrt(der[kd+1]*der[kd+1]+der[kd+2]*der[kd+2]+
		   ...
		   elem_avderz[nelem*nc2+ixcell] += fabs(der[kd+3])/(double)ns3;
		   elem_maxderz[nelem*nc2+ixcell] =
		     std::max(elem_maxderz[nelem*nc2+ixcell], fabs(der[kd+3]));
		}
	    }
	}
    }
...
// Sequential 
   for (int kr=0; kr<nc2; ++kr)
      ...
      for (int ka=0; ka<num_elem; ++ka)
	{
	  ...
	  avderz[kr] += elem_avderz[ka*nc2 + kr];
	  maxderz[kr] = std::max(maxderz[kr], elem_maxderz[ka*nc2+kr]);
	}
    }
